...
data = []
SOLR_URL = 'http://localhost:8983/solr/testingcore/select'

# Query_1 Logic, Display All Based On Comment_Author
def query_1(search_term=None, start_date=None, end_date=None, sentiment_categories=None):

    # Default Parameters
    params = {
        'q': f'{search_term}' if search_term else "*:*", #Term to search
        'q.op' : "AND",
        'fl': 'submission_score, title, comment_date, comment_content, sentiment_score, sentiment_category',  # Fields to return
    ...
    # Add in sentiment filter
    if sentiment_categories:
        sentiment_filter = ' OR '.join([f'sentiment_category:"{category}"' for category in sentiment_categories])
        if 'fq' in params:
            params['fq'] += f' AND ({sentiment_filter})'
        else:
            params['fq'] = f'({sentiment_filter})'
    ...
    try:
        ...
        processed_data = []
        processed_data.append(['submission_score', 'title', 'comment_date', 'comment_content', 'sentiment_score', 'sentiment_category'])
        for entry in data:
            processed_entry = []
            for field in ['submission_score', 'title', 'comment_date', 'comment_content', 'sentiment_score', 'sentiment_category']:
...
def index():
    ...
    if request.method == 'POST':
        ...
        sentiment_categories = request.form.getlist('sentiment_category')
    ...
    filtered_data = query_function(search_term, start_date, end_date, sentiment_categories)
